...
for i in range(64):
    ...
    plt.title(f'Index = {i + 1}', fontsize=8)
...
plt.tight_layout()
plt.savefig('64_plots.png', dpi=150, bbox_inches='tight')
print("Saved: 64_plots.png")
plt.close()

# Find Best and Worst based on KL distance
kl = np.array(kl)
KLIdx = np.argsort(kl)
worstidx = np.sort(KLIdx[:8])
bestidx = np.sort(KLIdx[56:64])

print(f"\nBest 8 features (highest KL divergence): {bestidx + 1}")
print(f"Worst 8 features (lowest KL divergence): {worstidx + 1}")
...
for i in range(8):
    ...
    plt.plot(x, y_cheetah, label='Cheetah')
    plt.plot(x, y_grass, label='Grass')
    plt.title(f'Best {idx_b + 1}', fontsize=10)
    ...
    plt.plot(x, y_cheetah, label='Cheetah')
    plt.plot(x, y_grass, label='Grass')
    plt.title(f'Worst {idx_w + 1}', fontsize=10)
    plt.legend(fontsize=8)

plt.tight_layout()
plt.savefig('BestandWorst_plots.png', dpi=150, bbox_inches='tight')
print("Saved: BestandWorst_plots.png")
plt.close()
...
# Save prediction
Image.fromarray((Blocks * 255).astype(np.uint8)).save('prediction_64d.jpg')
print("Saved: prediction_64d.jpg")
...
# ===================================================================
# Problem c(ii): Use 8 best features
# ===================================================================
print("\n" + "=" * 60)
...
# Save prediction
Image.fromarray((Blocks * 255).astype(np.uint8)).save('prediction_8d.jpg')
print("Saved: prediction_8d.jpg")
